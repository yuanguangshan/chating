...
const fetchData = async (url) => {
  try {
    ...
    if (!response.ok) {
      console.error(
        `[期货数据] 对 ${url} 的API请求失败，状态码 ${response.status}`
      );
...
const getFuturesData = async () => {
  try {
    ...
    const marginDataMap = new Map(
      marginResponse.data.map((item) => [item.uniqueIdEx, item])
    );
    ...
    const processedData = zdfData
      ...
      .filter(Boolean);
    console.log(
      `[期货数据] 成功获取并处理了 ${processedData.length} 条期货数据。`
    );
...
const getPrice = async (name) => {
  try {
    ...
    if (contract) {
      ...
      console.log(
        `[获取价格] 找到合约 "${name}" (代码 "${symbol}"):`,
        priceInfo
      );
      return JSON.stringify(priceInfo);
    } else {
      console.warn(
        `[获取价格] 未找到名为 '${name}' (代码 '${symbol}') 的合约。`
      );
      return JSON.stringify({
        error: `未找到名为 '${name}' (代码 '${symbol}') 的合约。`,
      });
    }
  } catch (error) {
...
export { getFuturesData, getPrice };
