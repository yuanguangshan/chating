...
const MASTER_TOOL_DEFINITIONS = [
    ...
    { name: "query_aggregate", description: "聚合查询期货数据（如最高价、最低价、平均成交量等）", parameters: { type: "OBJECT", properties: { symbol: { type: "STRING", description: "品种代码如rb、cu等" }, days: { type: "INTEGER", description: "查询天数，默认5天" }, aggFunc: { type: "STRING", description: "聚合函数：MAX、MIN、AVG、SUM" }, column: { type: "STRING", description: "字段名：最高、最低、成交量等" } }, required: ["symbol", "aggFunc", "column"] } },
...
function formatHistoryForOpenAI(history) {
    ...
    return history
        .filter(msg => msg && typeof msg.text === 'string')
        .map(msg => {
            const role = KNOWN_AI_USERNAMES.includes(msg.username) ? 'assistant' : 'user';
...
async function callGeminiApi(modelUrl, payload, env, logCallback = () => {}) {
    ...
    if (keys.length === 0) throw new Error('服务器配置错误：未设置GEMINI_API_KEY。');
...
export async function getDeepSeekExplanation(text, env) {
    ...
    const now = new Date();
    const beijingTimeFormatter = new Intl.DateTimeFormat('en-US', { hour: 'numeric', minute: 'numeric', hour12: false, timeZone: 'Asia/Shanghai' });
    ...
    let modelToUse = (beijingHour === 0 && beijingMinute >= 31) || (beijingHour > 0 && beijingHour < 8) || (beijingHour === 8 && beijingMinute <= 29) ? "deepseek-coder" : "deepseek-chat";
    const response = await fetch("https://api.deepseek.com/chat/completions", {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${apiKey}` },
        body: JSON.stringify({ model: modelToUse, messages: [{ role: "system", content: "你是一个有用的，善于用简洁的markdown语言来解释下面的文本." }, { role: "user", content: text }], temperature: 0.8 })
    });
    if (!response.ok) {
        const errorText = await response.text();
...
export async function getKimiExplanation(text, env) {
    try {
        const data = await callKimiApi("moonshot-v1-8k", {
            messages: [{ role: "system", content: "你是一个有用的助手，善于用简洁的markdown语言来解释文本。" }, { role: "user", content: text }],
            temperature: 0.3,
        }, env);
        const explanation = data?.choices?.[0]?.message?.content;
...
export async function getGeminiChatAnswer(question, history = [], env, logCallback = () => {}) {
    ...
    const contents = [
        { role: "user", parts: [{ text: `你是一个全能的AI助手，可以处理金融期货、实时行情、新闻查询、图表生成等多种任务，同时也能回答通用问题。**回答规则：**1. **默认情况**：优先提供专业、准确的回答，特别是在金融期货领域。2. **通用问题**：如果用户的问题明显与金融无关（例如生活、科技、娱乐等），则提供通用回答。3. **特殊标记**：如果用户的问题以 "@@" 结尾，则无论问题内容如何，都提供通用回答（适用于用户明确希望脱离金融场景时）。4. **语气要求**：保持友好、清晰，避免过度技术术语（除非用户明确要求）。不要随便拒绝回答，记住，你什么都懂。` }] },
...
export async function getKimiChatAnswer(question, history = [], env, logCallback = () => {}) {
    ...
    const messages = [
        { role: "system", content: `你是一个全能的AI助手，可以处理金融期货、实时行情、新闻查询、图表生成等多种任务，同时也能回答通用问题。**回答规则：**1. **默认情况**：优先提供专业、准确的回答，特别是在金融期货领域。2. **通用问题**：如果用户的问题明显与金融无关（例如生活、科技、娱乐等），则提供通用回答。3. **特殊标记**：如果用户的问题以 "@@" 结尾，则无论问题内容如何，都提供通用回答（适用于用户明确希望脱离金融场景时）。4. **语气要求**：保持友好、清晰，避免过度技术术语（除非用户明确要求）。不要随便拒绝回答，记住，你什么都懂。` },
        ...formattedHistory,
        { role: "user", content: question }
    ];
    try {
        const data = await callKimiApi("moonshot-v1-8k", { messages, temperature: 0.3, tools, tool_choice: "auto" }, env, logCallback);
        ...
        if (choice.finish_reason === "tool_calls" && choice.message.tool_calls) {
            const toolResults = await Promise.all(choice.message.tool_calls.map(async (toolCall) => {
                ...
                try { args = JSON.parse(argsString); } catch (e) {
                    ...
                    return { tool_call_id: toolCall.id, role: "tool", content: JSON.stringify({ error: `工具参数解析失败: ${e.message}` }) };
                }
                ...
                try {
                    ...
                    return { tool_call_id: toolCall.id, role: "tool", content: JSON.stringify({ content: result }) };
                } catch (e) {
                    logCallback(`❌ [Tool Error] Kimi function ${name} failed: ${e.message}`, 'ERROR', e);
                    return { tool_call_id: toolCall.id, role: "tool", content: JSON.stringify({ error: `工具执行失败: ${e.message}` }) };
                }
            }));
            const finalMessages = [ ...messages, choice.message, ...toolResults ];
            const finalData = await callKimiApi("moonshot-v1-8k", { messages: finalMessages, temperature: 0.3 }, env, logCallback);
...
export async function getDeepSeekChatAnswer(question, history = [], env, logCallback = () => {}) {
    ...
    try {
        ...
        const response = await fetch("https://api.deepseek.com/chat/completions", {
            method: "POST",
            headers: { "Content-Type": "application/json", "Authorization": `Bearer ${apiKey}` },
            body: JSON.stringify({ model: modelToUse, messages, temperature: 0.3, tools, tool_choice: "auto" })
        });
        if (!response.ok) {
            const errorText = await response.text();
        ...
        if (choice.finish_reason === "tool_calls" && choice.message.tool_calls) {
            ...
            const finalResponse = await fetch("https://api.deepseek.com/chat/completions", {
                method: "POST",
                headers: { "Content-Type": "application/json", "Authorization": `Bearer ${apiKey}` },
                body: JSON.stringify({ model: modelToUse, messages: finalMessages, temperature: 0.3 })
            });
            if (!finalResponse.ok) {
                const errorText = await finalResponse.text();
                throw new Error(`DeepSeek API错误: ${finalResponse.status} ${errorText}`);
            }
