...
export function formatAggregateResult(data, symbol, days, aggFunc, column) {
    if (!data || !data.data || data.data.result === null || data.data.result === undefined) {
        return `未找到${symbol}过去${days}天的${column}${aggFunc === 'MAX' ? '最高' : 
               aggFunc === 'MIN' ? '最低' : 
               aggFunc === 'AVG' ? '平均' : ''}值`;
    }

    const value = data.data.result;
    const unit = column === '成交量' ? '手' : column === '成交额' ? '元' : '元/吨';
    
    return `${symbol}品种过去${days}天的${column}${aggFunc === 'MAX' ? '最高' : 
           aggFunc === 'MIN' ? '最低' : 
           aggFunc === 'AVG' ? '平均' : ''}值为 ${value}${unit}`;
}
...
export async function smartQuery(query) {
    const { parseNaturalQuery, validateQuery, generateQueryDescription } = await import('./naturalQueryParser.js');
    ...
    try {
        ...
        switch (parsed.type) {
            case 'aggregate':
                result = await getAggregateData(parsed.symbol, parsed.days, parsed.aggFunc, parsed.column);
                formattedResult = formatAggregateResult(result, parsed.symbol, parsed.days, parsed.aggFunc, parsed.column);
                break;
