...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (
        pathname === '/api/internal-task-handler' &&
        request.method === 'POST'
      ) {
        const task = await request.json()
        // [修正] 将具体的派发逻辑移到独立的函数中，保持 fetch 函数的整洁
        ctx.waitUntil(dispatchInternalTask(task, env))
        return new Response('Internal task accepted by worker.', {
          status: 202,
        })
      }
      ...
      if (
        pathname.startsWith('/api/toutiao/') ||
        pathname === '/api/inspirations/generate'
      ) {
        console.log(`[Worker] Routing to ToutiaoDO: ${pathname}`)
        if (!env.TOUTIAO_SERVICE_DO)
          throw new Error("Durable Object 'TOUTIAO_SERVICE_DO' is not bound.")
      ...
      if (pathname.startsWith('/api/zhihu/')) {
        ...
        if (!env.ZHIHU_SERVICE_DO)
          throw new Error("Durable Object 'ZHIHU_SERVICE_DO' is not bound.")
      ...
      if (pathname.startsWith('/api/inspirations')) {
        ...
        if (!env.INSPIRATION_DO)
          throw new Error("Durable Object 'INSPIRATION_DO' is not bound.")
      ...
      if (pathname.startsWith('/api/messages/history')) {
        ...
        // 对于 API 请求，我们通常需要一个特定的 DO 实例，这里假设是 'default' 或从路径中解析
        // 但历史消息通常是针对特定房间的，所以需要从 URL 中提取房间名
        // 尝试从路径中提取房间名，如果路径是 /api/messages/history/roomName
        const roomNameMatch = pathname.match(
          /^\/api\/messages\/history\/([a-zA-Z0-9_-]+)/,
        )
        let roomName = roomNameMatch ? roomNameMatch[1] : null

        // 如果路径中没有房间名，则尝试从查询参数中获取
        if (!roomName) {
      ...
      return new Response('Not Found. Check worker routing.', {
        status: 404,
        headers: corsHeaders,
      })
    } catch (err) {
  ...
  try {
    ...
    switch (command) {
      case 'toutiao_article':
        if (!env.TOUTIAO_SERVICE_DO)
          throw new Error('Toutiao Service DO is not configured.')
        serviceStub = env.TOUTIAO_SERVICE_DO.get(
          env.TOUTIAO_SERVICE_DO.idFromName('toutiao-singleton'),
        )
      ...
      case 'inspiration':
        if (!env.INSPIRATION_DO)
          throw new Error('Inspiration Service DO is not configured.')
        serviceStub = env.INSPIRATION_DO.get(
          env.INSPIRATION_DO.idFromName('inspiration-singleton'),
        )
      ...
      case 'zhihu_article':
        if (!env.ZHIHU_SERVICE_DO)
          throw new Error('Zhihu Service DO is not configured.')
        serviceStub = env.ZHIHU_SERVICE_DO.get(
          env.ZHIHU_SERVICE_DO.idFromName('zhihu-singleton'),
        )
    ...
    console.log(
      `[Worker] Task "${command}" successfully dispatched to ${serviceName}.`,
    )
  } catch (e) {
...
async function handleErrorCallback(error, callbackInfo, env) {
  if (!callbackInfo || !callbackInfo.roomName || !callbackInfo.messageId) {
    console.error(
      '[Worker] FATAL: Cannot perform error callback due to missing callbackInfo.',
      { error, callbackInfo },
    )
    return
  }
  try {
    ...
    console.error(
      `[Worker] FATAL: Error callback to room ${callbackInfo.roomName} failed!`,
      callbackError,
    )
  }
}
...
function serveHtmlWithEnv(htmlContent, env, pageType = 'main') {
  let injectedScript = `window.ENV_CONFIG = ${JSON.stringify({
    ...
    managementRoomsList: (env.MANAGEMENT_ROOMS_LIST || 'general,test')
      .split(',')
      .map(r => r.trim()),
  })};\n`
