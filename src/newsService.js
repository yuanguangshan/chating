export async function fetchNewsFromTongHuaShun() {
  try {
    ...
    console.log(
      `[NewsService] Successfully processed ${mappedData.length} TongHuaShun news items.`
    );
    return mappedData;
  } catch (error) {
...
export async function fetchNewsFromDongFangCaiFu() {
  try {
    ...
    console.log(
      `[NewsService] Successfully processed ${mappedData.length} DongFangCaiFu news items.`
    );
    return mappedData;
  } catch (error) {
...
export async function getNews(keyword) {
  try {
    ...
    if (filteredNews.length > 0) {
      console.log(
        `[getNews] Found ${filteredNews.length} news items for keyword "${keyword}".`
      );
      ...
      console.log(
        `[getNews] No news found for "${keyword}". Returning ${fallbackNews.length} top news items as fallback.`
      );
