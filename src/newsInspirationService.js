...
class NewsInspirationService {
  constructor(env = null) {
    ...
    this.apiBaseUrl =
      env?.YOUR_EXTERNAL_NEWS_API_BASE_URL ||
      "https://newsnow.want.biz/api/s?id=";
    ...
    const mockData = {
      hupu: {
        items: [
          {
            id: "h1",
            title: "NBA季后赛激战正酣，湖人vs掘金抢七大战",
            url: "https://bbs.hupu.com/1",
            hotValue: 50000,
          },
          {
            id: "h2",
            title: "国足世预赛最新动态：国足1-0小胜泰国",
            url: "https://bbs.hupu.com/2",
            hotValue: 35000,
          },
          {
            id: "h3",
            title: "CBA总决赛：辽宁vs广东巅峰对决",
            url: "https://bbs.hupu.com/3",
            hotValue: 28000,
          },
        ],
      },
      weibo: {
        items: [
          {
            id: "w1",
            title: "#高考放榜# 各省高考分数线陆续公布",
            url: "https://weibo.com/1",
            hotValue: 80000,
          },
          {
            id: "w2",
            title: "#夏日限定美食# 网红冰淇淋测评",
      ...
      douyin: {
        items: [
          {
            id: "d1",
            title: "爆火舞蹈挑战：科目三全网模仿",
            url: "https://douyin.com/1",
            hotValue: 65000,
          },
          {
            id: "d2",
            title: "美食博主探店：隐藏版深夜食堂",
            url: "https://douyin.com/2",
            hotValue: 38000,
          },
          {
            id: "d3",
            title: "萌宠视频：猫咪的搞笑日常",
            url: "https://douyin.com/3",
            hotValue: 25000,
          },
        ],
      },
      hackernews: {
        items: [
          {
            id: "hn1",
            title: "OpenAI发布GPT-4o最新版本",
            url: "https://news.ycombinator.com/1",
            hotValue: 45000,
          },
          {
            id: "hn2",
            title: "Linux内核6.9发布，性能大幅提升",
      ...
      nowcoder: {
        items: [
          {
            id: "nc1",
            title: "字节跳动2025校招启动，算法岗竞争激烈",
            url: "https://nowcoder.com/1",
            hotValue: 40000,
          },
          {
            id: "nc2",
            title: "LeetCode周赛：双周赛第108题解",
          ...
          {
            id: "nc4",
            title: "编程入门：如何选择第一门编程语言",
            url: "https://nowcoder.com/4",
            hotValue: 15000,
          },
          {
            id: "nc5",
            title: "数据结构与算法精讲",
            url: "https://nowcoder.com/5",
            hotValue: 12000,
          },
        ],
      },
  ...
  async #fetchData(sourceId) {
    ...
    if (
      this.cache[sourceId] &&
      now - this.cache[sourceId].timestamp < this.cacheDuration
    ) {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(
          `[NewsInspirationService] HTTP error for ${sourceId}: ${response.status} ${response.statusText}`
        );
      ...
      if (!contentType || !contentType.includes("application/json")) {
        console.warn(
          `[NewsInspirationService] Invalid content type for ${sourceId}: ${contentType}`
        );
        return this.useMockData ? this.#getMockData(sourceId) : { items: [] };
      }
      ...
      try {
        ...
        console.warn(
          `[NewsInspirationService] Invalid JSON for ${sourceId}: ${jsonError.message}`
        );
        console.warn(
          `[NewsInspirationService] Response preview: ${text.substring(0, 200)}...`
        );
        return this.useMockData ? this.#getMockData(sourceId) : { items: [] };
      }
      ...
      if (!data || !Array.isArray(data.items)) {
        console.warn(
          `[NewsInspirationService] Invalid response structure for ${sourceId}:`,
          data
        );
        return this.useMockData ? this.#getMockData(sourceId) : { items: [] };
      }
      ...
      console.log(
        `[NewsInspirationService] Successfully fetched ${data.items.length} items from ${sourceId}`
      );
      ...
      console.error(
        `[NewsInspirationService] Failed to fetch ${sourceId}: ${error.message}`
      );
  ...
  #normalizeHackerNews(rawData) {
    ...
    return rawData.items.map((item) => ({
      ...
      url: item.url || "#",
      source: "HackerNews",
      hotValue: item.extra?.info
        ? parseInt(item.extra.info.replace(" points", ""))
  ...
  #getNormalizedItems(sourceId, rawData) {
    ...
    switch (sourceId) {
      case "hupu":
        return this.#normalizeHupu(rawData);
      case "douyin":
        return this.#normalizeDouyin(rawData);
      case "weibo":
        return this.#normalizeWeibo(rawData);
      case "hackernews":
        return this.#normalizeHackerNews(rawData);
      case "nowcoder":
        return this.#normalizeNowcoder(rawData);
      default:
        return [];
    }
  }
  ...
  async getCombinedNewsInspiration() {
    ...
    results.forEach((result, index) => {
      ...
      if (result.status === "fulfilled" && result.value) {
        ...
        switch (sourceId) {
          case "hupu":
            normalizedItems = this.#normalizeHupu(data);
            break;
          case "douyin":
            normalizedItems = this.#normalizeDouyin(data);
            break;
          case "weibo":
            normalizedItems = this.#normalizeWeibo(data);
            break;
          case "hackernews":
            normalizedItems = this.#normalizeHackerNews(data);
            break;
          case "nowcoder":
            normalizedItems = this.#normalizeNowcoder(data);
            break;
          default:
            console.warn(
              `[NewsInspirationService] Unknown source ID: ${sourceId}`
            );
        }

        console.log(
          `[NewsInspirationService] Normalized ${normalizedItems.length} items from ${sourceId}`
        );
        allNews.push(...normalizedItems);
        sourceCounts[sourceId] = normalizedItems.length;
      } else if (result.status === "rejected") {
        console.error(
          `[NewsInspirationService] Failed to fetch data from ${sourceId}:`,
          result.reason
        );
        ...
        if (this.useMockData) {
          ...
          console.log(
            `[NewsInspirationService] Using mock fallback for ${sourceId}: ${normalizedItems.length} items`
          );
          allNews.push(...normalizedItems);
          sourceCounts[sourceId] = normalizedItems.length;
        }
    ...
    if (allNews.length === 0) {
      console.warn(
        `[NewsInspirationService] No news items found from any source, using full mock data`
      );
    ...
    for (const newsItem of allNews) {
      const normalizedTitle = newsItem.title
        .toLowerCase()
        .replace(/[^a-z0-9\u4e00-\u9fa5]/g, ""); // Include Chinese chars
    ...
    console.log(
      `[NewsInspirationService] Combined and deduplicated ${deduplicatedNews.length} news items.`
    );
    console.log(
      `[NewsInspirationService] Returning ${deduplicatedNews.length} items.`
    );
