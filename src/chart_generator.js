...
function getChart1Option(dataList) {
  const processedData = dataList.map((item) => {
    const openChange = parseFloat(
      (((item.o - item.zjsj) / item.zjsj) * 100).toFixed(2)
    );
    const closeChange = parseFloat(
      (((item.p - item.zjsj) / item.zjsj) * 100).toFixed(2)
    );
    const lowChange = parseFloat(
      (((item.l - item.zjsj) / item.zjsj) * 100).toFixed(2)
    );
    const highChange = parseFloat(
      (((item.h - item.zjsj) / item.zjsj) * 100).toFixed(2)
    );
    return {
  ...
  const sortedData = processedData.sort(
    (a, b) => b.closeChange - a.closeChange
  );
  ...
  if (sortedData.length > 0 && sortedData[0].closeChange > 0) {
    markPointData.push({
      name: "涨幅最高",
      value: sortedData[0].closeChange,
      coord: [0, sortedData[0].value[1]],
      itemStyle: { color: "#ef4444" },
    });
  }
  if (
    sortedData.length > 1 &&
    sortedData[sortedData.length - 1].closeChange < 0
  ) {
    markPointData.push({
      name: "跌幅最大",
      value: sortedData[sortedData.length - 1].closeChange,
      coord: [
        sortedData.length - 1,
        sortedData[sortedData.length - 1].value[1],
      ],
      itemStyle: { color: "#22c553" },
    });
  }
  return {
    ...
    series: [
      {
        ...
        type: "bar",
        yAxisIndex: 0,
        data: dataList.map((item) => item.tjd),
      },
      {
        name: "涨跌幅",
        type: "line",
        yAxisIndex: 1,
        data: dataList.map((item) => item.zdf),
      },
    ],
  };
}
    ...
    series: [
      {
        ...
        data: dataList.map((item) => {
          ...
          return divisor === 0
            ? 0
            : parseFloat(((item.rz / divisor) * 100).toFixed(2));
        }),
      },
      {
        name: "涨跌幅",
...
function getChart4Option(dataList) {
  ...
  return {
    ...
    series: seriesConfig.map((s) => ({
      ...
      type: "line",
      smooth: true,
      data: dataList.map((item) => item[s.field]),
    })),
  };
}
...
function getChart5Option(dataList, metric1, metric2) {
  ...
  return {
    title: {
      text: `${fieldMapping[metric1]}与${fieldMapping[metric2]}对比`,
      left: "center",
    },
    backgroundColor: "transparent",
    tooltip: { trigger: "axis" },
    ...
    series: [
      {
        ...
        type: "bar",
        yAxisIndex: 0,
        data: dataList.map((item) => item[metric1]),
      },
      {
        name: fieldMapping[metric2],
        type: "line",
        yAxisIndex: 1,
        data: dataList.map((item) => item[metric2]),
      },
    ],
  };
}
...
function getChart6Option(dataList) {
  ...
  const treemapData = dataList.filter(
    (item) => item.cje > 10 * 1e8 && typeof item.zdf === "number"
  );
...
function getChart7Option(dataList) {
  const scatterData = dataList
    .map((item) => {
      const prevCcl = item.ccl - item.rz;
      const rzRatio =
        prevCcl === 0 ? 0 : parseFloat(((item.rz / prevCcl) * 100).toFixed(2));
      return {
        name: getSimplifiedName(item.name),
    ...
    .filter(
      (item) => item.value[0] !== undefined && item.value[1] !== undefined
    );
  ...
  return {
    ...
    visualMap: {
      type: "continuous",
      dimension: 2, // 默认 cdzj (index 2)
      min: minSizeMetric,
      max: maxSizeMetric,
      show: false,
      inRange: { symbolSize: [30, 100] },
    },
    series: [
      {
        type: "scatter",
...
function getChart8Option(dataList) {
  ...
  const avgRiseZdf =
    totalRiseCount > 0 ? (sumRiseZdf / totalRiseCount).toFixed(2) : "0.00";
  const displayAvgFallZdf =
    totalFallCount > 0
      ? (Math.abs(sumFallZdf) / totalFallCount).toFixed(2)
      : "0.00";
...
async function generateAndUploadChart(env, chartTitle, option, width, height) {
  ...
  await env.R2_BUCKET.put(r2Key, svg, {
    httpMetadata: { contentType: "image/svg+xml" },
  });
  console.log(
    `[ChartGenerator] SVG chart "${fileName}" generated and uploaded to R2 at ${r2Key}.`
  );
...
export async function drawChart(env, symbol, period = "daily") {
  ...
  try {
    ...
    if (!klineResponse.ok) {
      throw new Error(
        `Failed to fetch k-line data for ${symbol}: ${klineResponse.statusText}`
      );
    }
    ...
    if (!klineData || klineData.length === 0) {
      return JSON.stringify({
        error: `No k-line data found for ${symbol} with period ${period}.`,
      });
    }
    ...
    const option = {
      title: {
        text: `${symbol.toUpperCase()} ${period} K-Line Chart`,
        left: "center",
      },
    ...
    console.log(
      `[drawChart] Successfully generated chart for ${symbol} (${period}): ${url}`
    );
    return JSON.stringify(result);
  } catch (error) {
    console.error(`[drawChart] Failed to generate chart for ${symbol}:`, error);
    return JSON.stringify({
      error: `Failed to generate chart for ${symbol}. See logs for details.`,
    });
  }
}
...
export async function generateAndPostCharts(env, roomName) {
  ...
  try {
    ...
    if (!dataResponse.ok) {
      console.error(
        "[ChartGenerator] Failed to fetch data source from https://q.want.biz/"
      );
      throw new Error("Failed to fetch data source.");
    }
    const apiData = await dataResponse.json();
    console.log(
      `[ChartGenerator] Successfully fetched initial data. Total items: ${apiData.list.length}`
    );
    ...
    if (filteredData.length === 0) {
      console.log(
        "[ChartGenerator] No data meets the criteria (cdzj > 15亿), skipping chart generation."
      );
      return;
    }
    ...
    if (filteredData.length === 0) {
      console.log(
        "[ChartGenerator] No active data after session filtering, skipping chart generation."
      );
      return;
    }

    console.log(
      `[ChartGenerator] Data fetched and filtered. ${filteredData.length} items remaining.`
    );

    const chartsToGenerate = [
      {
        title: "期货品种走势对比",
        optionFunc: getChart1Option,
        width: 1500,
        height: 600,
      },
    ...
    const uploadPromises = chartsToGenerate.map((chartDef) => {
      const option = chartDef.optionFunc(
        filteredData,
        chartDef.metric1,
        chartDef.metric2
      );
      return generateAndUploadChart(
        env,
        chartDef.title,
        option,
        chartDef.width,
    ...
    console.error(
      `[ChartGenerator] Process failed: ${error.message}`,
      error.stack || error
    );
  }
}
