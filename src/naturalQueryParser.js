...
export function parseNaturalQuery(query) {
  ...
  if (
    ...
    normalized.includes("平均") ||
    normalized.includes("总和")
  ) {
    ...
    normalized.includes("日线") ||
    normalized.includes("日k") ||
...
function extractSymbol(text) {
  ...
  if (codeMatch) {
    ...
    if (Object.values(SYMBOL_MAP).includes(code)) {
      return {
        code,
        name: Object.keys(SYMBOL_MAP).find((k) => SYMBOL_MAP[k] === code),
      };
    }
  }
