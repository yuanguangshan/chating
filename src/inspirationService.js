...
export class InspirationService {
     ...
     */
    async getCombinedInspirations(limits = { zhihuHot: 10, zhihuInspiration: 10 }) {
        ...
        const [zhihuResult, newsResult] = await Promise.allSettled([
            this.zhihuService.getCombinedTopics(limits.zhihuHot, limits.zhihuInspiration),
        ...
        if (zhihuResult.status === 'fulfilled' && zhihuResult.value) {
            ...
            const zhihuHotNormalized = hotTopics.map(item => this._normalizeZhihuItem(item, '知乎热点'));
            const zhihuInspirationNormalized = inspirationQuestions.map(item => this._normalizeZhihuItem(item, '知乎灵感'));
            
            combinedList.push(...zhihuHotNormalized, ...zhihuInspirationNormalized);
            console.log(`[InspirationService] ✅ 成功整合 ${hotTopics.length} 个知乎热点和 ${inspirationQuestions.length} 个知乎灵感。`);
        } else if (zhihuResult.status === 'rejected') {
            console.error('[InspirationService] ❌ 获取知乎数据失败:', zhihuResult.reason);
        }

        // 2. 处理新闻数据
        if (newsResult.status === 'fulfilled' && newsResult.value) {
            const newsItems = newsResult.value;
            const newsNormalized = newsItems.map(item => this._normalizeNewsItem(item));
            combinedList.push(...newsNormalized);
            console.log(`[InspirationService] ✅ 成功整合 ${newsItems.length} 个新闻热点。`);
        } else if (newsResult.status === 'rejected') {
            console.error('[InspirationService] ❌ 获取新闻数据失败:', newsResult.reason);
        }
        ...
        console.log(`[InspirationService] ✨ 整合完成，共返回 ${deduplicatedList.length} 条高质量灵感。`);
